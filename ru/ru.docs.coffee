_app.i18n.ru.docs = 
  title: 'Документация'
  meta:
    description: 'Как использовать и настроить ostr.io? Как получать СМС-сообщения обо всех входах (логинах) через SSH, и когда Ваш сервер недоступен? Как контролировать нагрузку сервера и предотвратить долгий ответ сервера?'
  basics:
  ...
  SSLcertificate:
    ...
    Для мониторинга целостности SSL (TLS) сертификата и его срока действия, во время добавления сервера, укажите `https` протокол, пример: `https://example.com`. Как только SSL сертификат обнаружен и подтвержден - Ostr.io будет регулярно сверять его отпечаток. В разделе SSL-сертификата укажите контакты, которые будут уведомлены о нарушении целостности и приближающейся дате окончания действия сертификата, о которой Ostr.io напомнит за месяц, неделю и сутки.
    """

  domainExpiration:
    title: 'Уведомления о сроках регистрации домена'
    text: """
    При указанной дате окончания срока действия доменного имени в Whois записи (*доступно на 98% доменов*), включите уведомления для всех ответственных лиц (*полезно для владельцев доменного имени*). Ostr.io уведомит Вас за месяц, неделю и сутки до окончания регистрации доменного имени.
  ...
  addEmail:
    ...
    Далее, для подтверждения адреса - проследуйте по ссылке полученной в письме от Ostr.io. Как только адрес будет подтвержден, он будет доступен для добавления в списки уведомлений.
    """
  ...
  servers:
    ...
    - __Файл__ - Добавьте файл с указанным содержанием в корневую директорию Вашего вебсайта, содержание файла должно начинаться с `ostrio-domain: `. Данный способ удобен при наличии доступа через FTP;
    - __DNS-запись__ - Добавьте TXT запись с указанны содержанием к основному доменному имени (`@`). Запись должна начинаться с `ostrio-domain: `. Данный способ удобен при наличии доступа к управлению DNS записями
    - __Заголовок ответа__ - Добавьте пользовательский заголовок ответа с указанным содержанием. Ключ заголовка должен быть `x-ostrio-domain`. Данный способ удобен при наличии доступа к web- или proxy-серверу.
  ...
  endpoints:
    ...
    - __Пре-рендеринг__ - Если Ваше веб-приложение работает через "hashbang" `#!`, Ostr.io рекомендует добавить следующий параметр: `/path?_escaped_fragment_=`;
    - __Защищенная точка мониторинга__ - Ostr.io поддерживает точки мониторинга защищенные парой логин/пароль. Ostr.io рекомендует использовать защищенные пути, особенно при возврате *пользовательских данных*;
  ...
  json:
    ...
    text: """
    Для мониторинга и анализа пользовательских данных, возвращаемых точкой мониторинга, Ostr.io поддерживает формат JSON. Перед добавлением точки мониторинга в панели управления - точка мониторинга уже должна быть настроена на возврат необходимых данных. Рекомендуем заранее закрыть такую точку парой логин/пароль.
  ...
  xml:
    ...
    text: """
    Для мониторинга и анализа пользовательских данных, возвращаемых точкой мониторинга, Ostr.io поддерживает формат XML. Перед добавлением точки мониторинга в панели управления - точка мониторинга уже должна быть настроена на возврат необходимых данных. Рекомендуем заранее закрыть такую точку парой логин/пароль.
  ...
  ssh:
    ...
    - Для сбора данных Ostr.io настоятельно рекомендует использовать отдельного пользователя, который может исполнять __только__ следующие команды, если у пользователя нет прав на исполнение таких команд - данные собираться не будут, но мониторинг и доступность сервера через SSH будет осуществляться в обычном режиме:
      - `top`;
      - `grep`;
      - `tail`;
      - `awk`;
