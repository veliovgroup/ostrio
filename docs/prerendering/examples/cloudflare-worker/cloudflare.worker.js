...
export default {
  async fetch(request, env) {
    ...
    const userAgent = request.headers.get(STRS.headers.ua)?.toLowerCase?.() || STRS.stringEmpty;
    const pathName = url.pathname.toLowerCase();
    const extension = pathName.substring(pathName.lastIndexOf(STRS.dot) + 1 || pathName.length)?.toLowerCase?.() || STRS.stringEmpty;

    if (!url.origin.includes(STRS.service.origin) && !url.pathname.includes(STRS.wellknownPath) && (userAgent && BOT_AGENTS_RE.test(userAgent)) && !(extension && IGNORE_EXTENSIONS.includes(extension))) {
      ...
      // COMMENT OUT TO DISABLE RENDERING FOR URLs WITH ?getquery=value
      if (typeof url.search === STRS.string && url.search.length > 1) {
        fetchUrl += url.search;
      }
